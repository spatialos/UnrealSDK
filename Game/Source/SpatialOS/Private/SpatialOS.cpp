...
void USpatialOS::ApplyConfiguration(const FSOSWorkerConfigurationData& InWorkerConfigurationData)
{
  checkf(!IsConnected(), TEXT("ApplyConfiguration was called after Connect was called."));
  WorkerConfiguration = FSOSWorkerConfiguration(InWorkerConfigurationData);
}
...
void USpatialOS::ApplyEditorWorkerConfiguration(FWorldContext& InWorldContext)
{
  ...
  if (EditorConfigurationArrayIndex < NumWorkerConfigurations)
  {
    ...
    WorkerConfiguration = FSOSWorkerConfiguration(WorkerConfig.WorkerConfigurationData);
...
const FSOSWorkerConfiguration USpatialOS::GetWorkerConfiguration() const
{
...
UEntityPipeline* USpatialOS::GetEntityPipeline()
{
